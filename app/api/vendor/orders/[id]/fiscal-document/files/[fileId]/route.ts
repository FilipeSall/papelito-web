...
export async function GET(
  _request: Request,
  { params }: { params: Promise<{ fileId: string; id: string }> },
) {
  ...
  try {
    response = await fetch(
      `${base}/papelito/v1/vendor/me/orders/${id}/fiscal-document/files/${fileId}`,
      {
        cache: "no-store",
    ...
    return NextResponse.json({ message: "Não foi possível abrir o arquivo da nota." }, { status: 502 });
  }
  ...
  return new NextResponse(response.body, {
    headers: {
      ...
      "Content-Disposition": response.headers.get("content-disposition") ?? `attachment; filename="nota-${fileId}"`,
