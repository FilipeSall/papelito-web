...
export async function GET() {
  const session = await readWithVendorAccessToken((accessToken) =>
    wpRest<VendorPendingRegistrationResponse>("/papelito/v1/vendor/registration-pending", {
...
export async function POST(request: Request) {
  ...
  const result = await wpRest<VendorPendingRegistrationResponse>("/papelito/v1/vendor/registration-pending", {
    method: "POST",
    headers: {
