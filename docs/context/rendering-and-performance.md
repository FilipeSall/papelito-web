...
**Filtrar não é aparecer na navegação.** A barra de chips e as rotas `/premium`, `/novidades` e
`/promocoes` continuam com a lista fixa: são superfícies de espaço limitado, e criar a décima coleção
não pode desmontar o layout. Coleção nova é navegável por `?colecao=<slug>`, não ganha chip
automático.
