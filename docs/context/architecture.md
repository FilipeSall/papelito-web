...
Do sistema do corredor da vitrine pública, descrito em [../brand/identidade-visual.md](../brand/identidade-visual.md#11-sistema-do-corredor--vitrine-pública): `ShelfLabel` (etiqueta de gôndola, tons `dark`/`yellow`), `Shelf` (trilho horizontal com encaixe, setas sobrepostas e régua) e `ScribbleRule` (régua desenhada com rough.js). `SectionHeader` foi removido — era usado só pelas duas seções da home e virou `ShelfLabel`.
