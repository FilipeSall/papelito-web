...
- **A fonte da região é o CEP fiscal da empresa B2B ativa, quando existir; caso contrário, usa o CEP salvo na conta.** Sem prompt, sem cookie, sem store de CEP — esse desenho foi implementado e removido. O CEP fiscal tem precedência para disponibilidade regional.
...
- **Componente que estoura a complexidade cognitiva se divide** (`typescript:S3776`, teto 15). No React o estouro quase nunca vem de um `if`: vem do JSX. Um `.map()` que renderiza um cartão inteiro é uma função aninhada e conta junto com o componente — extraia `<ItemCard />`, `<ItemFacts />`, `<ItemActions />` em vez de encolher a lógica. Cada `? :`, cada `&&` e cada cadeia de `||` dentro do JSX soma; conditional que só decide "mostra ou não" vira `if (!x) return null;` no componente filho.
- **Handler assíncrono repetido vira um só corpo parametrizado.** Três `fetch` com a mesma forma — trava de duplo clique, leitura tolerante do erro, `router.refresh()` no sucesso — são um helper com `{ url, method, body, successMessage, failureMessage, onSuccess }`, não três blocos gêmeos. Reduz S3776 e faz a diferença entre as ações aparecer em dados.
- **Função que não usa estado nem props sai do componente** (`typescript:S7721`). `fetch` embrulhado, regex de validação, formatador: vão para o escopo do módulo, onde não se recriam a cada render e podem ser testados sozinhos. O que fica dentro é o que lê `useState`, `props` ou `router`.
- **Classe Tailwind arbitrária que já tem token não entra** (`tailwindcss:suggestCanonicalClasses`). `tracking-[0.1em]` é `tracking-widest`; confira o valor em `app/globals.css` antes de trocar, porque parte dos tokens é redefinida lá (`--tracking-wide` é `0.075em`, não o padrão). Colchete só para valor que o tema realmente não tem.
