...
export function SectionHeader({ section }: Readonly<{ section: AdminSectionKey }>) {
  ...
  return (
    <section className={section === "products" ? "grid gap-4" : "grid gap-4 xl:grid-cols-[1.6fr_0.8fr]"}>
      <Panel className="overflow-hidden">
        <div className="border-b border-[#231f20]/10 bg-[#231f20] px-5 py-3 text-brand-yellow md:px-6">
        ...
        <div className="relative px-5 py-6 md:px-6 md:py-7">
          ...
          <div
            aria-hidden
            className="absolute right-[5%] top-[12%] hidden h-48 w-48 rounded-full border border-brand-yellow/12 bg-[#fff8b7]/24 blur-3xl md:block"
          />
          <div
            aria-hidden
            className="absolute bottom-[-18%] right-[20%] hidden h-44 w-44 rounded-full bg-brand-yellow/8 blur-[72px] md:block"
          />
          <div className="relative max-w-3xl">
            ...
            <div className="mt-6 grid gap-3 md:grid-cols-3">
              {HERO_METRICS[section].map((metric) => (
                <div
                  key={metric.label}
                  className="rounded-xl border border-[#231f20]/12 bg-white/82 px-4 py-3"
                >
