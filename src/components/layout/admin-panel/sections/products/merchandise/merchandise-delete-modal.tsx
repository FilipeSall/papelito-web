...
type MerchandiseDeleteModalProps = Readonly<{
  blockingKits: AdminMerchandiseKitUsage[];
  deleting: boolean;
  error: string;
...
export function MerchandiseDeleteModal({
  blockingKits,
  deleting,
  error,
  merchandise,
  onCancel,
  onConfirm,
}: MerchandiseDeleteModalProps) {
  useEscapeKey(onCancel, { enabled: !deleting });

  const inUse = merchandise.kitCount > 0 || blockingKits.length > 0;
  const kits = blockingKits.length > 0 ? blockingKits : merchandise.kits;

  return (
    <div
      aria-labelledby="merchandise-delete-title"
      aria-modal="true"
    ...
    >
      ...
      >
        ...
        <div className="space-y-4 px-5 py-5 text-sm leading-6 text-[#5e574c]">
          <p>
            Você vai excluir{" "}
            <strong className="text-[#231f20]">{merchandise.name}</strong> do
            catálogo de brindes.
          </p>
          {inUse ? (
            <div className="border-2 border-[#c0392b] bg-[#fff0ed] px-3 py-2 text-xs leading-5 text-[#8b1f16]">
              <p className="font-bold">
                Este brinde ainda é usado e não pode ser excluído. Remova-o destes
                Kits primeiro:
              </p>
              <ul className="mt-2 space-y-1">
                {kits.map((kit) => (
                  <li key={kit.kitId}>· {kit.name}</li>
                ))}
              </ul>
            </div>
          ) : (
            <p className="border-2 border-[#1a1a1a] bg-brand-yellow px-3 py-2 text-xs font-bold leading-5 text-[#231f20]">
              A imagem será apagada do storage se nenhum outro conteúdo a usar.
            </p>
          )}
          {error && !inUse ? (
            <p
              className="border-2 border-[#c0392b] bg-[#fff0ed] px-3 py-2 text-xs text-[#8b1f16]"
              role="alert"
            >
              {error}
            </p>
        ...
        <footer className="flex justify-end gap-3 border-t-2 border-[#1a1a1a] bg-white px-5 py-4">
          ...
          {inUse ? null : (
            ...
            >
              ...
              {deleting ? "Excluindo…" : "Excluir brinde"}
            </button>
          )}
        </footer>
      </section>
