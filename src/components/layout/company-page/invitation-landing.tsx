 ...
 *    cookie HttpOnly e devolve o preview (empresa, CNPJ e papel). O token some da URL. O CNPJ é
 *    exibido a partir da relação convite → empresa; o convidado nunca o digita nem o escolhe.
...
export function InvitationLanding({ token }: InvitationLandingProps) {
  ...
  const handleAccept = useCallback(async () => {
    ...
    try {
      ...
      if (!result.ok) {
        if (result.code === "papelito_b2b_invitation_identity_required") {
          router.push(`/convite/cpf?callbackUrl=${encodeURIComponent(RETURN_PATH)}`);
          return;
        }
        setMessage(`⚠ ${result.message}`);
        setPhase("error");
      ...
      setMessage("⚠ Não foi possível concluir o vínculo com a empresa. Tente novamente.");
      setPhase("error");
    }
  }, [router, update]);
  ...
  useEffect(() => {
    ...
    return () => window.clearTimeout(acceptTimer);
  }, [emailMatches, handleAccept, isAuthed, phase, preview]);
      ...
      <div className="border-2 border-[#1a1a1a] bg-[#faf8f2] p-8 shadow-[8px_8px_0px_#1a1a1a]">
        ...
        {preview && (phase === "ready" || phase === "accepting" || phase === "error") ? (
          <div className="space-y-4">
            ...
            {preview.companyCnpj ? (
              <p className="border-2 border-[#1a1a1a] bg-white px-3 py-2 text-[13px] font-medium text-[#231f20]">
                CNPJ da empresa:{" "}
                <strong className="font-black">{formatCnpj(preview.companyCnpj)}</strong>
              </p>
            ) : null}
